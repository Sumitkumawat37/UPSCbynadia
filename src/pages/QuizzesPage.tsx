        ...
        {quizzes.map((quiz, i) => (
          <Card
            ...
            className={`p-4 transition-all ${
              quiz.status === "available" ? "cursor-pointer hover:card-shadow-lg hover:scale-[1.01] active:scale-[0.99]" : ""
            }`}
            onClick={() => quiz.status === "available" && navigate(`/quiz/${quiz.id}`)}
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {quiz.status === "available" && <ChevronRight className="w-4 h-4 text-muted-foreground" />}
              </div>
            </div>
          </Card>
        ))}
