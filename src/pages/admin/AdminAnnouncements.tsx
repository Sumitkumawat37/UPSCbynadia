...
const AdminAnnouncements = () => {
  ...
  const handleSend = () => {
    ...
    createAnnouncement.mutate({ title, message, type: "info" });
  ...
  return (
    <div className="space-y-4 animate-slide-up">
      ...
      <Card className="p-4 space-y-3">
        <h3 className="font-semibold text-sm flex items-center gap-2"><Megaphone className="w-4 h-4 text-primary" /> New Announcement</h3>
        <Input placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} />
        <Textarea placeholder="Message to all students..." value={message} onChange={(e) => setMessage(e.target.value)} rows={3} />
        <Button className="w-full" onClick={handleSend}><Send className="w-4 h-4 mr-2" /> Send to All Students</Button>
      </Card>
