...
import { useCourses, useLectures, useNotes } from "@/lib/supabase-data";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Video, FileText, Plus, Upload, BookOpen, Eye } from "lucide-react";
      ...
      <Tabs defaultValue="courses">
        ...
        <TabsContent value="courses" className="space-y-3 mt-3">
          <Dialog open={showCourseForm} onOpenChange={setShowCourseForm}>
            <DialogTrigger asChild><Button className="w-full"><Plus className="w-4 h-4 mr-2" /> Create New Course</Button></DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>Create New Course</DialogTitle></DialogHeader>
              <div className="space-y-3">
                <div className="space-y-1"><Label className="text-xs">Course Title</Label><Input placeholder="e.g. Advanced Mathematics" /></div>
                <div className="space-y-1"><Label className="text-xs">Description</Label><Textarea placeholder="Course description..." rows={2} /></div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1"><Label className="text-xs">Price (₹)</Label><Input type="number" placeholder="999" /></div>
                  <div className="space-y-1"><Label className="text-xs">Category</Label><Input placeholder="e.g. Mathematics" /></div>
                </div>
                <div className="space-y-1"><Label className="text-xs">Instructor Name</Label><Input placeholder="e.g. Rajesh Kumar" /></div>
                <div className="space-y-1"><Label className="text-xs">Thumbnail Image URL</Label><Input placeholder="https://..." /></div>
                <Button className="w-full" onClick={() => { toast.success("Course created — use admin API for full CRUD"); setShowCourseForm(false); }}>Create Course</Button>
              </div>
            </DialogContent>
          </Dialog>
          {courses.map((c) => (
        ...
        <TabsContent value="videos" className="space-y-3 mt-3">
          <Dialog open={showLectureForm} onOpenChange={setShowLectureForm}>
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                <div className="space-y-1"><Label className="text-xs">YouTube Video Link</Label><Input placeholder="https://youtube.com/watch?v=..." /><p className="text-[10px] text-muted-foreground">Paste full YouTube URL. It will be auto-embedded.</p></div>
                <Button className="w-full" onClick={() => { toast.success("Lecture added"); setShowLectureForm(false); }}>Add Lecture</Button>
              </div>
            </DialogContent>
          </Dialog>
          {lectures.map((l) => (
        ...
        <TabsContent value="notes" className="space-y-3 mt-3">
          <Dialog open={showNoteForm} onOpenChange={setShowNoteForm}>
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                <div className="space-y-1"><Label className="text-xs">Note Title</Label><Input placeholder="e.g. Formula Sheet" /></div>
                <Button className="w-full" onClick={() => { toast.success("Notes uploaded"); setShowNoteForm(false); }}>Upload Notes</Button>
              </div>
            </DialogContent>
          </Dialog>
          {notes.map((n) => (
